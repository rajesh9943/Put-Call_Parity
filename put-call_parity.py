...
T = st.number_input("Time to Expiration (T) in years", min_value=0.0, value=1.0, step=0.25)
...
if T > 0:
    ...
    # If T = 0, option is expiring now → PV(K) = K
    P = C - S + K
...
> If the actual market put price differs significantly, it may indicate an arbitrage opportunity [[6]].
""")